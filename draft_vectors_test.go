...
func TestDraftVectors(t *testing.T) {
	vectors := []struct {
		...
		{"percent-encoded-at-userinfo", "obfuscate", "http://user%40attacker.example", "[http]://user[@]attacker[.]example"},
		{"percent-encoded-non-delim", "obfuscate", "http://%65vil.example", "[http]://%65vil[.]example"},
		{"double-encoded-not-decoded", "obfuscate", "http://evil%252eexample", "[http]://evil%252eexample"},
